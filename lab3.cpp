...

{
    ...
    float x = 0, y, num, denom;//Переменная с плавующей запятой
    cout<< " Впишіть число: ";//Показ в консоли 
    cin>> x;//Считывает данные с консоли
   
    num = cbrt(abs(pow(x,2)/2 - pow(sin(pow(x,3)),2))) * (log(abs(x)))/log(2.5);//Чеслитель задания
    denom = pow(2,x) * sqrt(abs(x + 2.5) * abs(tan(x)));//Знаменатель задания
    y = num/denom;//Деление Чеслителя на Знаменатель
